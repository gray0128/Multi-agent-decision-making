...
本地、可审计、可恢复的多 Agent 审议工具。`mad` 通过用户已经安装并认证的 AI CLI 执行一次性非交互调用，支持结构化审议、自由讨论、透明档案和独立观察页。
...
- 只能响应检查点，不能发起、恢复、删除审议或修改配置。

guided 审议在没有交互终端时需要一个在线观察服务，否则会立即失败而不是无限等待。
...
- 观察服务不直接写正式档案，只通过一次性文件信箱响应检查点。

不要通过反向代理、端口转发或隧道暴露观察服务，也不要把 CLI 凭证写入 `clis.toml`。
...
`mad serve` 默认使用系统分配的空闲端口并自动打开带临时令牌的本地观察页；如果系统浏览器无法启动，终端会保留可手动打开的完整地址。`--port` 接受 `0` 到 `65535`，其中 `0` 仍表示自动分配。`resume` 只允许选择输出格式，不能改变原审议的模式、交互策略、方案或预算。
